
module.exports = (app,session,Post,queryString ,postsController)=>{

 app.get('/posts/:id', postsController.Postspage);

/*******************************************************************************************************************************

             Crear nuevo post de usuario 

**********************************************************************************************************************************/ 

 app.post('/posts/new', postsController.postingNew);



/***********************************************************************************************************************************************
 * 
 *           Todos los posts de un usuario
 * 
 * **********************************************************************************************************************************************/

app.get('/user/posts/json/:id', postsController.Userposts);



/*************************************************************************************************************************************

         Todos los posts de los usuarios  sin ningun orden


***************************************************************************************************************************************/


  app.get('/all/posts/json/:id', postsController.Allposts);


/*************************************************************************************************************************************************
 * 
 *             Filtrar posts según  categoria
 * 
 * ***********************************************************************************************************************************************/    


// app.post('/all/posts/json', postsController.notAllposts);


/*************************************************************************************************************************************************
 * 
 *             Ordenar los posts por raiting o ranking  y si tienen categoria tambien 
 * 
 * ***********************************************************************************************************************************************/    


 app.post('/all/posts/raiting/json',postsController.filterByraiting);



  /****************************************************************************************************************************************
   * 
   *  Página de un  solo post donde irán los comentarios
   * 
   * **************************************************************************************************************************************/

 
  app.get('/post/:id', postsController.OnepostPage);



  /*********************************************************************************************************************************************************+
   *
   *  Cargar todos los comentarios  de un post enformato JSON
   *  
   ********************************************************************************************************************************************************************/ 
   
    app.get('/post/data/json/:id',postsController.OnePostData);


  /****************************************************************************************************************************************+
   *     Añadir comentarios al Post
   * 
   * ***************************************************************************************************************************************/

   app.post('/add/comment/post/:id',postsController.addCommentPost);


  /*************************************************************************************************************************************************
   *                  Votar por un post
   * 
   * ***********************************************************************************************************************************************/

 app.post('/post/vote' ,postsController.votePost);


/***************************************************************************************************************************************************
 * 
 *                        Borrar Post
 * 
 **************************************************************************************************************************************************/


 app.delete('/post/delete/:id', postsController.deletePost);




}


